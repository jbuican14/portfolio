const PastWork = () => {
  ...
  return (
    <section id="pastWork" className="py-12 px-6 lg:py-20 lg:px-20 flex items-center">
      <div className="w-full max-w-6xl mx-auto">
        <h2 className="mb-12">Past Work & Achievements</h2>
        <p className="text-lg mb-8 italic text-gray-600">Projects during my time at ad agency (Verizon Media/AOL)</p>

        <div className="space-y-8">
          {projects.map((project, index) => (
            <div key={index} className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-8">
              <a
                href={project.link}
                target="_blank"
          ...
          <ul className="space-y-4">
            {achievements.map((achievement, index) => (
              <li key={index} className="flex items-start">
                <i className={`fa ${achievement.icon} text-yellow-500 text-2xl mr-4 mt-1`}></i>
                <span className="text-lg" dangerouslySetInnerHTML={{ __html: achievement.text.replace('1st', '1<sup>st</sup>') }}></span>
              </li>
            ))}
          </ul>
        </div>
